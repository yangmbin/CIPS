...
<%
    ...
    ResultSet rs=stmt.executeQuery("SELECT* FROM comment where iid='"+ID+"'");//执行SQL语句并返回结果.
	...
	if(!rs.next())
	{
		...
		rs=stmt.executeQuery("SELECT* FROM comment where iid='"+ID+"'");//执行SQL语句并返回结果.
	...
	%>
		...
		<fieldset>
			...
			<div style="text-align:right">评论人：<%=rs.getString("username")%></div>
		</fieldset>
		<br />
		<%
		rs.next();
		...
		<%if(curPage>1){%><a href="comment.jsp?ID=<%=ID%>&page=1">首页</a><%}%>&nbsp;&nbsp;

		<%if(curPage>1){%><a href="comment.jsp?ID=<%=ID%>&page=<%=curPage-1%>">上一页</a><%}%>&nbsp;&nbsp;

		<%
		for(int j=1;j<=totalPage;j++)
		{
			out.print("&nbsp;&nbsp;<a href='comment.jsp?ID="+ID+"&page="+j+"'>"+j+"</a>");
		}
		%>

		&nbsp;&nbsp;
		<%if(curPage<totalPage){%><a href="comment.jsp?ID=<%=ID%>&page=<%=curPage+1%>">下一页
		</a><%}%>&nbsp;&nbsp;

		<%if(totalPage>1){%><a href="comment.jsp?ID=<%=ID%>&page=<%=totalPage%>">末页</a><%}%>
	<%
	}
	...
	<form action="comment_store.jsp?ID=<%=ID%>" method='post'> 
	<p>评论：</p>
