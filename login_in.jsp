...
<%
	...
	if(!rs.next())
	{
		...
		rs=stmt.executeQuery("SELECT* FROM information");//执行SQL语句并返回结果.
	...
	%>
		...
		<fieldset>
			...
			<div style="text-align:right">发表于：<%=rs.getString("date")%> | 作者：<%=rs.getString("username")%>
			<a href="comment.jsp?ID=<%=rs.getInt("id")%>">评论</a></div>
		</fieldset>
		<br />
		<%
		rs.next();
		...
		<%if(curPage>1){%><a href="login_in.jsp?page=1">首页</a><%}%>&nbsp;&nbsp;

		<%if(curPage>1){%><a href="login_in.jsp?page=<%=curPage-1%>">上一页</a><%}%>&nbsp;&nbsp;

		<%
		for(int j=1;j<=totalPage;j++)
		{
			out.print("&nbsp;&nbsp;<a href='login_in.jsp?page="+j+"'>"+j+"</a>");
		}
		%>

		&nbsp;&nbsp;
		<%if(curPage<totalPage){%><a href="login_in.jsp?page=<%=curPage+1%>">下一页
		</a><%}%>&nbsp;&nbsp;

		<%if(totalPage>1){%><a href="login_in.jsp?page=<%=totalPage%>">末页</a><%}%>
	<%
	}
	%>
